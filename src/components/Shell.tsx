...
function GroqKeyModal({ open, onClose }: { open: boolean; onClose: () => void }) {
...
function AvatarMenu() {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex h-9 w-9 items-center justify-center rounded-full border border-indigo-400/40 bg-indigo-500/20 font-mono text-[12px] font-bold text-indigo-200 transition-all hover:border-indigo-300/70 hover:shadow-glow"
      >
        {initials}
      </button>
      {open && (
        <div className="anim-pop absolute right-0 top-12 w-60 card rounded-xl! p-2 z-50">
          <div className="border-b border-white/[0.07] px-3 pb-3 pt-2">
            ...
            <span className="mono-tag mt-1.5 inline-block text-violet-400">{user.role}</span>
          </div>
          <button
            onClick={() => {
              setOpen(false);
            ...
            className="mt-1 flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-[13.5px] font-medium text-slate-300 transition-colors hover:bg-white/[0.06] hover:text-white"
          >
            <Icon name="spark" size={15} className="text-indigo-400" /> AI engine settings
          </button>
          <button
            onClick={() => {
              setOpen(false);
            ...
            className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-[13.5px] font-medium text-slate-300 transition-colors hover:bg-rose-500/10 hover:text-rose-300"
          >
            <Icon name="logout" size={15} className="text-rose-400" /> Sign out
          </button>
        </div>
      )}
...
function GenerationOverlay() {
  ...
  const stage = GENERATION_STAGES[generating.stageIndex] ?? GENERATION_STAGES[0];
  return (
          ...
          {GENERATION_STAGES.map((_, i) => (
            <span
              ...
              className={cls(
                ...
                i < generating.stageIndex ? "bg-indigo-500" : i === generating.stageIndex ? "bg-violet-400 striped-bar" : "bg-slate-700/60"
              )}
            />
          ))}
        </div>
        <p className="mt-6 text-xs text-slate-500">Usually takes under 15 seconds — hang tight.</p>
      </div>
    </div>
  );
}
      ...
      <header className="sticky top-0 z-40 border-b border-white/[0.06] bg-ink-950/75 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6">
          ...
          <nav className="hidden items-center gap-1 md:flex">
            <Link
              ...
              className={cls(
                ...
                location.pathname === "/dashboard" ? "bg-white/[0.07] text-white" : "text-slate-400 hover:text-white"
              )}
            >
          ...
          <div className="flex items-center gap-3">
            <Button size="sm" icon="plus" onClick={() => navigate("/projects/new")} className="px-3.5!">
