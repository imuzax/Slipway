    ...
    <div className="grid gap-5 lg:grid-cols-3">
      <Reveal className="lg:col-span-2">
        <div className="card p-6">
          ...
          <p className="mt-3 text-[14.5px] leading-relaxed text-slate-700 dark:text-slate-300">
            {p.description}
          </p>

          {p.problemStatement && (
            <div className="mt-5 rounded-xl border border-rose-500/20 bg-rose-50 p-4 dark:bg-rose-500/[0.06]">
              <p className="flex items-center gap-2 text-[13px] font-bold text-rose-600 dark:text-rose-300">
          ...
          {p.targetAudience && (
            <div className="mt-4 rounded-xl border border-emerald-500/20 bg-emerald-50 p-4 dark:bg-emerald-500/[0.06]">
              <p className="flex items-center gap-2 text-[13px] font-bold text-emerald-600 dark:text-emerald-300">
          ...
          <div className="card p-6">
            ...
            <div className="mt-3 space-y-3.5">
              {stack
                ...
                .map(([label, items]) => (
                  <div key={label}>
                    <p className="mb-1.5 font-mono text-[10.5px] uppercase tracking-wider text-slate-500">
                      {label}
                    </p>
            ...
            <dl className="mt-3 space-y-2.5 text-sm">
              {[
                ...
                >
                  ...
                  <dd className="font-semibold capitalize text-slate-900 dark:text-slate-200">
                    {v}
                  </dd>
                </div>
              ))}
        ...
        <div className="card p-6">
          <div className="flex items-center justify-between">
            <p className="mono-tag text-indigo-400">
              // {p.features.length} planned features
            </p>
          ...
          <ul className="mt-4 grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
            {p.features.map((f, i) => (
              <li
                ...
                "
              >
                <span className="font-mono text-[10.5px] font-bold text-violet-600 dark:text-violet-400">
    ...
    return (
      ...
      >
        <Button
          ...
          icon="spark"
          disabled={!!generating}
          onClick={() => void generateRoadmap(p.id)}
        >
    ...
    <div className="grid gap-5 lg:grid-cols-3">
      <div className="card p-6 lg:col-span-2">
        ...
        <div className="mt-5 space-y-4">
          ...
          <div className="flex justify-end border-t border-slate-200 pt-5 dark:border-white/[0.07]">
            <Button
              icon="check"
              onClick={() => {
      ...
      <div className="card h-fit border-rose-300 bg-rose-50 p-6 dark:border-rose-500/25 dark:bg-transparent">
        <p className="mono-tag text-rose-500 dark:text-rose-400">
          // danger zone
        </p>
        ...
        <p className="mt-2 text-sm leading-relaxed text-slate-700 dark:text-slate-400">
          ...
          , its roadmap and all tracked progress. This cannot be undone.
        </p>
      ...
      >
        <p className="leading-relaxed text-slate-700 dark:text-slate-300">
          ...
          and its roadmap ({p.progress}% complete) will be permanently removed
          from this browser.
        </p>
      </Modal>
    </div>
...
export default function ProjectDetail() {
  ...
  return (
    <div>
        ...
        <div className="mt-4 flex flex-wrap items-start justify-between gap-6">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-3">
              <h1 className="font-display text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl dark:text-white">
                {p.projectName}
              </h1>
      ...
      {/* tabs */}
      <Reveal delay={120}>
        <div className="mt-8 flex gap-1.5 border-b border-slate-200 dark:border-white/[0.07]">
          {TABS.map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
            ...
            >
              <Icon
                name={t.icon}
                size={15}
                className={tab === t.id ? "text-indigo-400" : ""}
              />

              {t.label}

              {tab === t.id && (
                <span className="absolute inset-x-2 -bottom-px h-[2.5px] rounded-full bg-gradient-to-r from-indigo-500 to-violet-400" />
              )}
            </button>
          ))}
        </div>
