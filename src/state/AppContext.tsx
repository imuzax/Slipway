...
function derivePhaseStatus(
  prev: PhaseStatus,
  tasks: Roadmap["phases"][number]["tasks"],
): PhaseStatus {
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const user = useMemo(
    () => users.find((u) => u.id === session) ?? null,
    [users, session],
  );
  ...
  const myRoadmaps = useMemo(
    () => (user ? roadmaps.filter((r) => r.userId === user.id) : []),
    [roadmaps, user],
  );
  ...
  const roadmapFor = useCallback(
    (projectId: string) => roadmaps.find((r) => r.projectId === projectId),
    [roadmaps],
  );
    ...
    (projectId: string, phaseId: string, status: PhaseStatus) => {
      applyRoadmapUpdate(projectId, (rm) => ({
        ...
        phases: rm.phases.map((ph) => {
          ...
          let tasks = ph.tasks;
          if (status === "completed")
            tasks = tasks.map((t) => ({ ...t, status: "done" as TaskStatus }));
          if (status === "not-started")
            tasks = tasks.map((t) => ({ ...t, status: "todo" as TaskStatus }));
          if (
            status === "in-progress" &&
            !tasks.some((t) => t.status !== "todo")
          ) {
            tasks = tasks.map((t, i) =>
              i === 0 ? { ...t, status: "in-progress" as TaskStatus } : t,
            );
          }
  ...
  const generateRoadmap = useCallback(
    async (projectId: string): Promise<Roadmap | null> => {
      ...
      const work = (async (): Promise<{
        content: ReturnType<typeof buildLocalRoadmapContent>;
        ...
        if (groqKey) {
          try {
            ...
            toast(
              "info",
              "Groq was unreachable — falling back to the offline chart engine.",
            );
          }
        }
        await sleep(600);
        return {
          content: buildLocalRoadmapContent(project),
      ...
      commitRoadmaps([
        ...roadmapsRef.current.filter((r) => r.projectId !== projectId),
        rm,
      ]);
      commitProjects(
        projectsRef.current.map((p) =>
  ...
  const saveGroqKey = useCallback(
    (key: string) => {
      ...
      toast(
        "success",
        key.trim()
          ? "Groq key saved — generations now use llama-3.1-70b."
          : "Groq key removed — using the offline chart engine.",
      );
    },
    [toast],
  );
